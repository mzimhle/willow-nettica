...
require_once 'includes/recruiter_auth.php';
...
$uploadPath	= $_SERVER['DOCUMENT_ROOT'].'/client/'.$zfsession->domainData['clientType_directory'].'/'.$zfsession->domainData['fk_reference'].'/media/footer';
$imageURI	= 'http://'.$zfsession->domainData['recruiter_domain'].'/media/footer';

define('NICUPLOAD_PATH', $uploadPath); // Set the path (relative or absolute) to
                                      // the directory to save image files
                                      
define('NICUPLOAD_URI', $imageURI);   // Set the URL (relative or absolute) to
...
if($_SERVER['REQUEST_METHOD']=='POST') { // Upload is complete
    ...
    /* $filename = $id.'.'.$ext; */
	$filename = md5($id.rand(100000,1000000000000000)).'.'.$ext;
	...
	if((isset($size[0]) && $size[0] > 770) || (isset($size[1]) && $size[1] > 200)) {
		nicupload_error('The uploaded file\'s width/height is too big - max width is 770 and max height is 180');
	}
